...
//					+ Data (0)
  ...
  TH1F* npTPair_genX     = new TH1F("npTPair_gen","",21,-0.15,3.);
  TH1F* npTPair_recoX    = new TH1F("npTPair_reco","",21,-0.15,3.);
  TH1F* npTPairStab      = new TH1F("npTPair_stab","",21,-0.15,3.);
  TH1F* npTPairStabShift = new TH1F("npTPair_stabshift","",21,-0.15,3.);
  TH1F* npTPairPure      = new TH1F("npTPair_pure","",21,-0.15,3.);
  TH1F* npTPairPureShift = new TH1F("npTPair_pureshift","",21,-0.15,3.);
...
//	   && pt_pair < 0.8
	...
	if(nTrackExclu<1 
	  ...
	  double reso_pt0(-1), reso_pt1(-1), reso_pt01(-1);
          ...
          double gen_pt2=gen_dimuon.Pt();

          TLorentzVector reco_mu1, reco_mu2, reco_dimuon;
          reco_mu1.SetPtEtaPhiM(var_pt1[pair1],var_eta1[pair1],var_phi1[pair1],0.1057);
          reco_mu2.SetPtEtaPhiM(var_pt1[pair2],var_eta1[pair2],var_phi1[pair2],0.1057);
          reco_dimuon = reco_mu1 + reco_mu2;
          double reco_pt2=reco_dimuon.Pt();

          npTPair_genX->Fill(gen_pt2,weight); npTPair_recoX->Fill(reco_pt2,weight);

//------------------For purity and stability plots----------
	  if((int)(gen_pt2/(0.15)) == (int)(reco_pt2/0.15)) npTPairStab->Fill(gen_pt2,weight);
	  npTPairStabShift->Fill(gen_pt2,((int)(gen_pt2/(0.15))-(int)(reco_pt2/0.15))*weight);

	  if((int)(gen_pt2/(0.15)) == (int)(reco_pt2/0.15)) npTPairPure->Fill(reco_pt2,weight);
          npTPairPureShift->Fill(reco_pt2,(-(int)(gen_pt2/(0.15))+(int)(reco_pt2/0.15))*weight); 

//----------------------------------------------------------

//          MuMu3DAng1->Fill((mu11.Angle(mu12.Vect()))/pi,effcorrection1); 
...
TGraphErrors *pt2_stab = new TGraphErrors(20);
TGraphErrors *pt2_pure = new TGraphErrors(20);

TGraphErrors *pt2_stabShift = new TGraphErrors(20);
TGraphErrors *pt2_pureShift = new TGraphErrors(20);

for(int l=2; l<=21; l++){
  if(npTPair_genX->GetBinContent(l)>0){
        pt2_stab->SetPoint(l-2,-0.075+(l-1)*0.15,npTPairStab->GetBinContent(l)/(npTPair_genX->GetBinContent(l)));
        pt2_stab->SetPointError(l-2,0.075,0);
        pt2_stabShift->SetPoint(l-2,-0.075+(l-1)*0.15,npTPairStabShift->GetBinContent(l)/(npTPair_genX->GetBinContent(l)));
        pt2_stabShift->SetPointError(l-2,0.075,0);
  }
  else{pt2_stab->SetPoint(l-2,-1,-1); pt2_stabShift->SetPoint(l-1,-1,-1);}
  if(npTPair_recoX->GetBinContent(l)>0){
	pt2_pure->SetPoint(l-2,-0.075+(l-1)*0.15,npTPairPure->GetBinContent(l)/(npTPair_recoX->GetBinContent(l)));
	pt2_pure->SetPointError(l-2,0.075,0);
        pt2_pureShift->SetPoint(l-2,-0.075+(l-1)*0.15,npTPairPureShift->GetBinContent(l)/(npTPair_recoX->GetBinContent(l)));
        pt2_pureShift->SetPointError(l-2,0.075,0);
  }
  else{pt2_pure->SetPoint(l-2,-1,-1); pt2_pureShift->SetPoint(l-2,-1,-1);}
}
...
Correl->Divide(2,1);
Correl->cd(/*2*/1);
pt2_pure->SetMarkerStyle(20);
pt2_pure->SetMarkerSize(0.8);
pt2_pure->Draw("APL");
pt2_stab->SetMarkerStyle(20);
pt2_stab->SetMarkerSize(0.8);
pt2_stab->SetMarkerColor(4);
pt2_stab->Draw("PL,same");
Correl->cd(/*3*/2);
pt2_pureShift->SetMarkerStyle(20);
pt2_pureShift->SetMarkerSize(0.8);
pt2_pureShift->Draw("APL");
pt2_stabShift->SetMarkerStyle(20);
pt2_stabShift->SetMarkerSize(0.8);
pt2_stabShift->SetMarkerColor(4);
pt2_stabShift->Draw("PL,same");

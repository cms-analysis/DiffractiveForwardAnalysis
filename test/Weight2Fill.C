...
*/
 ...
 TH1F* fit_Inel =  new TH1F("fit_Inel","",20,0.,3.);
 TH1F* fit_others = new TH1F("fit_others","",20,0.,3.);
/*
...
// definitions des # d'entrées
  ...
  const float fac_lumi2 = 3.0525000e-7*integrated_lumi;                    //  3.05250e-6*integrated_lumi;
...
//Efficiency
  ...
  TFile f("MonteCarloHisto.root","recreate");
...
//	   && pt_pair < 0.8
	    ...
	    int nTrackExclu(0);
            for(Int_t j=0; j<nTrack; j++){  
		if(FailsTrackDistanceVeto(var_TrackD1[j], var_TrackQuality1[j], var_nHits1[j])){
                  filter1Track++;
		  nTrackExclu++;
		}
            }
	...
	if(nTrackExclu<1 
	  ...
	  pTPair1->Fill(pt_pair/**pt_pair*/,fac_lumi1*effcorrection1);
	  hEl->Fill(pt_pair/**pt_pair*/,fac_lumi1*effcorrection1);
          correl1->Fill(pt_pair/**pt_pair*/,fac_lumi1*effcorrection1);
          {fit_others->Fill(pt_pair/**pt_pair*/,fac_lumi1*effcorrection1);} 
...
//	   && pt_pair<0.8 
	    ...
	    int nTrackExclu(0);
            for(Int_t j=0; j<nTrack; j++){  
                if(FailsTrackDistanceVeto(var_TrackD2[j], var_TrackQuality2[j], var_nHits2[j])){ 
                  filter2Track++;
		  nTrackExclu++;
		}
            }
	...
	if(nTrackExclu<1 
		...
		&& PassesZDCVeto(var_zdcEmMinus2[0],var_zdcEmPlus2[0],var_zdcHadMinus2[0],var_zdcHadPlus2[0])
          ...
          MuMudphi2->Fill(1-(var_dphi2[0]/pi),fac_lumi2*effcorrection2); 
	  pTPair2->Fill(pt_pair/**pt_pair*/,fac_lumi2*effcorrection2);

  	          {correl2->Fill(pt_pair/**pt_pair*/,fac_lumi2*effcorrection2);}
		  double MassPcorrection(0),MassMcorrection(0);
/*		  	MassPcorrection = (var_MassYp2[0]>2) ? 1-(k*0.001)*sqrt(var_MassYp2[0]*var_MassYp2[0]-4) : 1;
		  	MassMcorrection = (var_MassYm2[0]>2) ? 1-(k*0.001)*sqrt(var_MassYm2[0]*var_MassYm2[0]-4) : 1;
		  	if(MassPcorrection<0) MassPcorrection=0;
                  	if(MassMcorrection<0) MassMcorrection=0;*/

//		        {extract[k]->Fill(pt_pair/**pt_pair*/,fac_lumi2*effcorrection2*MassPcorrection*MassMcorrection);}

			{fit_Inel->Fill(pt_pair/**pt_pair*/,fac_lumi2*effcorrection2/*MassPcorrection*MassMcorrection**/);}
	  } // if nTrack&nCalo if relevant
        }
  }
cout<<"InelEl :"<<endl;
...
//           && pt_pair<0.8
	    ...
	    int nTrackExclu(0);
            for(Int_t j=0; j<nTrack; j++){  
                if(FailsTrackDistanceVeto(var_TrackD3[j], var_TrackQuality3[j], var_nHits3[j])){  
                  filter3Track++;
		  nTrackExclu++;
		}
            }
	...
	if(nTrackExclu<1 
	  ...
	  pTPair3->Fill(pt_pair/* *pt_pair*/,fac_lumi3*effcorrection3);

                  {correl3->Fill(pt_pair/**pt_pair*/,fac_lumi3*effcorrection3);}
                  double MassPcorrection(0),MassMcorrection(0);
//                  for(int k=0; k<15; k++){
/*                        MassPcorrection = (var_MassYp3[0]>2) ? 1-(k*0.001)*sqrt(var_MassYp3[0]*var_MassYp3[0]-4) : 1;
                        MassMcorrection = (var_MassYm3[0]>2) ? 1-(k*0.001)*sqrt(var_MassYm3[0]*var_MassYm3[0]-4) : 1;
                        if(MassPcorrection<0) MassPcorrection=0;
                        if(MassMcorrection<0) MassMcorrection=0;*/

//			  {extract[k]->Fill(pt_pair/**pt_pair*/,fac_lumi3*effcorrection3);}
			{fit_others->Fill(pt_pair/**pt_pair*/,fac_lumi3*effcorrection3/**MassPcorrection*MassMcorrection*/);}
//			  }
...
//           && pt_pair<0.8
	    ...
	    int nTrackExclu(0);
            for(Int_t j=0; j<nTrack; j++){  
                if(FailsTrackDistanceVeto(var_TrackD4[j], var_TrackQuality4[j], var_nHits4[j])){   
                  filter4Track++;
		  nTrackExclu++;
		}
            }
...
//           && pt_pair<0.8
            ...
            int nTrackExclu(0);
            for(Int_t j=0; j<nTrack; j++){
                if(FailsTrackDistanceVeto(var_TrackD5[j], var_TrackQuality5[j], var_nHits5[j])){   
                  filter5Track++;
		  nTrackExclu++;
                }
            }
...
//           && pt_pair<0.8
            ...
            int nTrackExclu(0);
            for(Int_t j=0; j<nTrack; j++){
                if(FailsTrackDistanceVeto(var_TrackD6[j], var_TrackQuality6[j], var_nHits6[j])){   
                  filter6Track++;
		  nTrackExclu++;
                }
            }
            ...
            for(Int_t k=0; k<nCalo; k++){
               if(var_calodR6[k]>dRcone){
                                ...
                                if(var_caloId6[k]==6){
                                        if(var_caloHadE6[k]>HBThresh) nHB++;
                                        if(var_caloEmE6[k]>EBThresh) nEB++;}
                                if(var_caloId6[k]==7){
                                        if(var_caloHadE6[k]>HEThresh) nHE++;
                                        if(var_caloEmE6[k]>EBThresh) nEB++;}
                                if(var_caloId6[k]==8){
                                        if(var_caloHadE6[k]>HEThresh) nHE++;
                                        if(var_caloEmE6[k]>EEThresh) nEE++;}
               }
            }
		...
		{correl6->Fill(pt_pair/**pt_pair*/,fac_lumiBkg[bkgNum]*effcorrection6);}
	        {fit_others->Fill(pt_pair/**pt_pair*/,fac_lumiBkg[bkgNum]*effcorrection6);}
	  ...
	  pTPair6->Fill(pt_pair/**pt_pair*/,fac_lumiBkg[bkgNum]*effcorrection6);
          } // if nTrack&nCalo if relevant
...
/*DrawOneHistogramBis(sMuMuMass,MuMuMass0,MuMuMass1,MuMuMass2,MuMuMass3,MuMuMass4,MuMuMass5,MuMuMass6,"#mu#mu mass [GeV]","Events/1 GeV","MuMuMass_34pb-1_trackExclu2mm.png");
DrawOneHistogramBis(sMuMu3DAng,MuMu3DAng0,MuMu3DAng1,MuMu3DAng2,MuMu3DAng3,MuMu3DAng4,MuMu3DAng5,MuMu3DAng6,"#mu#mu 3D opening angle / #pi","Events/0.1","MuMu3dangle_34pb-1_trackExclu2mm.png");
DrawOneHistogramBis(sMuMudpt,MuMudpt0,MuMudpt1,MuMudpt2,MuMudpt3,MuMudpt4,MuMudpt5,MuMudpt6,"#mu#mu |events / 0.15 GeV| [GeV]","Events/0.1 GeV","MuMudpt_34pb-1_trackExclu2mm.root"); 
DrawOneHistogramBis(sMuMudphi,MuMudphi0,MuMudphi1,MuMudphi2,MuMudphi3,MuMudphi4,MuMudphi5,MuMudphi6,"#mu#mu 1-|#Delta #phi / #pi|","Events/0.01","MuMudphi_34pb-1_trackExclu2mm.root"); 
...
DrawOneHistogramBis(sMuMuDeta,MuMuDeta0,MuMuDeta1,MuMuDeta2,MuMuDeta3,MuMuDeta4,MuMuDeta5,MuMuDeta6,"#mu#mu #Delta #eta","Events/0.2","MuMuDeta_34pb-1_trackExclu2mm.png"); 
DrawOneHistogramBis(setaPair,etaPair0,etaPair1,etaPair2,etaPair3,etaPair4,etaPair5,etaPair6,"#eta(#mu#mu)","Events/0.2","etaPair_34pb-1_trackExclu2mm.png");  */
DrawOneHistogramBis(spTPair,pTPair0,pTPair1,pTPair2,pTPair3,pTPair4,pTPair5,pTPair6,"p_{T}(#mu#mu) [GeV]","Events/0.125 GeV","pTPair_34pb-1_trackExclu2mm.root"); /*  
...
DrawOneHistogramBis(spTSingleP,pTSingleP0,pTSingleP1,pTSingleP2,pTSingleP3,pTSingleP4,pTSingleP5,pTSingleP6,"p_{T}(#mu^{+}) [GeV]","Events/1 GeV","pTSingleP_34pb-1_trackExclu2mm.png");  
DrawOneHistogramBis(spTSingleM,pTSingleM0,pTSingleM1,pTSingleM2,pTSingleM3,pTSingleM4,pTSingleM5,pTSingleM6,"p_{T}(#mu^{-}) [GeV]","Events/1 GeV","pTSingleM_34pb-1_trackExclu2mm.png");
DrawOneHistogramBis(sphiSingleP,phiSingleP0,phiSingleP1,phiSingleP2,phiSingleP3,phiSingleP4,phiSingleP5,phiSingleP6,"#phi(#mu^{+})/#pi","Events/0.5","phiSingleP_34pb-1_trackExclu2mm.png");    
DrawOneHistogramBis(sphiSingleM,phiSingleM0,phiSingleM1,phiSingleM2,phiSingleM3,phiSingleM4,phiSingleM5,phiSingleM6,"#phi(#mu^{-})/#pi","Events/0.5","phiSingleM_34pb-1_trackExclu2mm.png");
DrawOneHistogramBis(sMuMuMassUps,MuMuMassUps0,MuMuMassUps1,MuMuMassUps2,MuMuMassUps3,MuMuMassUps4,MuMuMassUps5,MuMuMassUps6,"#mu#mu mass","Events/0.1 GeV","MuMuMassUps_34pb-1_trackExclu2mm.png");
...
DrawOneHistogramBis(sVtxT,VtxT0,VtxT1,VtxT2,VtxT3,VtxT4,VtxT5,VtxT6,"#mu#mu transverse vtx [cm]","Events/0.01 cm","VtxT_34pb-1_trackExclu2mm.png");
DrawOneHistogramBis(sVtxZ,VtxZ0,VtxZ1,VtxZ2,VtxZ3,VtxZ4,VtxZ5,VtxZ6,"#mu#mu z vtx [cm]","Events/2.5 cm","VtxZ_34pb-1_trackExclu2mm.png");
*/
...
Correl->Divide(3,2);
Correl->cd(1);
correl0->GetXaxis()->SetTitle("Data p_{T}(#mu#mu) [GeV]");
correl0->GetYaxis()->SetTitle("Data events / 0.15 GeV");
correl0->Draw();
Correl->cd(3);
correl1->GetXaxis()->SetTitle("El-El p_{T}(#mu#mu) [GeV]");
correl1->GetYaxis()->SetTitle("El-El events / 0.15 GeV");
correl1->Draw();
Correl->cd(4);
correl2->GetXaxis()->SetTitle("Inel-El p_{T}(#mu#mu) [GeV]");
correl2->GetYaxis()->SetTitle("Inel-El events / 0.15 GeV");
correl2->Draw();
Correl->cd(5);
correl3->GetXaxis()->SetTitle("Inel-Inel p_{T}(#mu#mu) [GeV]");
correl3->GetYaxis()->SetTitle("Inel-Inel events / 0.15 GeV");
correl3->Draw();
Correl->cd(6);
correl6->GetXaxis()->SetTitle("DY p_{T}(#mu#mu) [GeV]");
correl6->GetYaxis()->SetTitle("DY events / 0.15 GeV");
correl6->Draw();
Correl->SaveAs("correl.root");
...
Fit->Divide(4,1);
Fit->cd(1);
compatibility_a_N_90->GetXaxis()->SetTitle("a parameter");
compatibility_a_N_90->GetYaxis()->SetTitle("normalisation");
compatibility_a_N_90->Draw("colz");
Fit->cd(2);
compatibility_L_N_90->GetXaxis()->SetTitle("luminosity");
compatibility_L_N_90->GetYaxis()->SetTitle("normalisation");
compatibility_L_N_90->Draw("colz");
Fit->cd(3);
compatibility_a_L_90->GetXaxis()->SetTitle("a parameter");
compatibility_a_L_90->GetYaxis()->SetTitle("luminosity");
compatibility_a_L_90->Draw("colz");


cout<<"--------------"<<endl;
cout<<"Max Kolmo test: Prob="<<MinKolmo<<", a="<<-0.75+MinA*0.1<<", n="<<0.5+MinN*0.1<<", L="<<0.5+MinL*0.1<<endl;
*/

/*
 TH1F* hdataExtracted = (TH1F*) hdata->Clone("hdataExtracted");
 hdataExtracted->Add(fit_correl[MinA][MinN][MinL],-1);
 hdataExtracted->GetXaxis()->SetTitle("Data-Extract");
 Fit->cd(4);
 hdataExtracted->Draw();
Fit->SaveAs("fit.root");
*/

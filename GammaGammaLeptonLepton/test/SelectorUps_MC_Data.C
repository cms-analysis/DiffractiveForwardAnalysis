...
void SelectorUps_MC_Data() //NO PU, no use of Calo Tower, cumulative TGraph
{
gROOT->Reset();
...
// phi1-phi2 / pi
  ...
  TH1F *hmc_phi = new TH1F("hmc_phi","",50.,-0.1,0.1);
  TH1F *hinel_phi = new TH1F("hinel_phi","",50.,-0.1,0.1);
  TH1F *hdata_phi = new TH1F("hdata_phi","",50.,-0.1,0.1);
  ...
  TH1F *hmc_mass = new TH1F("hmc_mass","",80.,8.,12.);
  TH1F *hinel_mass = new TH1F("hinel_mass","",80.,8.,12.);
  TH1F *hdata_mass = new TH1F("hdata_mass","",80.,8.,12.);
...
// definitions des # d'entrées
  ...
  const double mass_upsilon_max = 12.0; //GeV
  const double mass_upsilon_min = 8.0; //GeV
	 ...
	 if(muID1==1 && muID2==1){
	   ...
	   if(var_b0[0]<dpt_max&&var_c0[0]>dphi_min&&var_d0[0]<vtxT_max&&var_f0[0]>closeTrk_min&&var_g0[0]<nextraCaloTower_max&&var_k0[0]<nhitZDC_max){
		...
		if(purity==false){
			...
			if(1.89*lumi_int <= 591){
	         		if(i_true < 1.89*lumi_int) {hdata_phi->Fill((phi0 - phi1)/Pi,1.); i_true++;
...
// Boucle #1
  ...
  for(Int_t i = 0;i < NUM1;i++){
		...
		if(purity == false){
	                ...
	                if(1.34*lumi_int <= 332){
				if(i_true < 1.34*lumi_int) {hdata_phi->Fill((phi0 - phi1)/Pi,1.); i_true++;
        ...
        if(hlt_pass==1){
	     ...
	     excl_11++;
             if(mass<=mass_upsilon_max&&mass>=mass_upsilon_min){
                mass_11++;
		if(purity==false){
                        double phi0 = var_h11[0] > 0 ? var_h11[0] : -var_h11[0];
                        double phi1 = (var_h11[1]+Pi) > 0 ? (var_h11[1]+Pi) : -(var_h11[1]+Pi);
                        phi1 = phi1 < Pi ? phi1 : fabs(phi1-2*Pi);
                        double dpt11 = var_b11[0];

                        if(1.33*lumi_int <= 161){
                                if(i_true < 1.33*lumi_int) {hdata_phi->Fill((phi0 - phi1)/Pi,1.); i_true++;
                                                            hdata_pt->Fill(dpt11,1.);
                                                            hdata_zdc->Fill(var_k11[0],1.);
                                                            hdata_castor->Fill(var_kk11[0],1.);
                                                            hdata_mass->Fill(mass,1.);}
                              i_exp++;
                        }
                        else {cout << "!!!!!!!!!!!" << endl;}
        ...
        if(hlt_pass==1){
	     ...
	     excl_12++;
             if(mass<=mass_upsilon_max&&mass>=mass_upsilon_min){
                mass_12++;
                if(purity==false){
                        double phi0 = var_h12[0] > 0 ? var_h12[0] : -var_h12[0];
                        double phi1 = (var_h12[1]+Pi) > 0 ? (var_h12[1]+Pi) : -(var_h12[1]+Pi);
                        phi1 = phi1 < Pi ? phi1 : fabs(phi1-2*Pi);
                        double dpt12 = var_b12[0];

                        if(0.567*lumi_int <= 205){
                                if(i_true < 0.567*lumi_int) {hdata_phi->Fill((phi0 - phi1)/Pi,1.); i_true++;
                                                            hdata_pt->Fill(dpt12,1.);
                                                            hdata_zdc->Fill(var_k12[0],1.);
                                                            hdata_castor->Fill(var_kk12[0],1.);
                                                            hdata_mass->Fill(mass,1.);}
                              i_exp++;
                        }
                        else {cout << "!!!!!!!!!!!" << endl;}
        ...
        if(hlt_pass==1){
	     ...
	     excl_13++;
             if(mass<=mass_upsilon_max&&mass>=mass_upsilon_min){
                mass_13++;
                if(purity==false){
                        double phi0 = var_h13[0] > 0 ? var_h13[0] : -var_h13[0];
                        double phi1 = (var_h13[1]+Pi) > 0 ? (var_h13[1]+Pi) : -(var_h13[1]+Pi);
                        phi1 = phi1 < Pi ? phi1 : fabs(phi1-2*Pi);
                        double dpt13 = var_b13[0];

                        if(0.536*lumi_int <= 217){
                                if(i_true < 0.536*lumi_int) {hdata_phi->Fill((phi0 - phi1)/Pi,1.); i_true++;
                                                            hdata_pt->Fill(dpt13,1.);
                                                            hdata_zdc->Fill(var_k13[0],1.);
                                                            hdata_castor->Fill(var_kk13[0],1.);
                                                            hdata_mass->Fill(mass,1.);}
                              i_exp++;
                        }
                        else {cout << "!!!!!!!!!!!" << endl;}
...
TCanvas *c_dpt = new TCanvas("dPt","delta(pT)");
   ...
   TPaveText *qed = new TPaveText(1.23322,56.021878,1.94354,62.50881,"br");
   qed->SetBorderSize(0);
   qed->SetFillColor(19);
   char text[500]; sprintf(text,"Upsilon signal: %g < m_{#mu#mu} < %g GeV",mass_upsilon_min,mass_upsilon_max);
   qed->AddText(text);

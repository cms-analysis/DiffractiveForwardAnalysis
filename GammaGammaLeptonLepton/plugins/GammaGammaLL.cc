...
GammaGammaLL::GammaGammaLL(const edm::ParameterSet& iConfig) :
  ...
  recoVertexToken_    (consumes< edm::View<reco::Vertex> >             (iConfig.getParameter<edm::InputTag>("RecoVertexLabel"))),
  muonToken_          (consumes< edm::View<pat::Muon> >                (iConfig.getParameter<edm::InputTag>("GlobalMuonCollectionLabel"))),
  eleToken_           (consumes< edm::View<pat::Electron> >            (iConfig.getParameter<edm::InputTag>("GlobalEleCollectionLabel"))),
...
std::cout << "looking at pairs: " << fetchElectrons_ << " (" << electronsMomenta.size() << ")/" << fetchMuons_ << " (" << muonsMomenta.size() << ")" << std::endl;
    PrimaryVertexSelector vtx(muonsMomenta, electronsMomenta);

    for (size_t i=0; i<recoVertexColl->size() && vtxind<MAX_VTX; i++) {
...
std::cout << "matched ele: " << vtx.matchedElectrons().size() << ", mu: " << vtx.matchedMuons().size() << std::endl;
	...
	if (ele_match.size()==0 or mu_match.size()==0) { // not enough muons or electrons candidates on the vertex
	  ...
	  const int lep1 = mu_match.at(i).first;
        ...
        l1.SetPtEtaPhiE(MuonCand_pt[Pair_lepton1[vtxind]],
                        MuonCand_eta[Pair_lepton1[vtxind]],
                        MuonCand_phi[Pair_lepton1[vtxind]],
                        MuonCand_e[Pair_lepton1[vtxind]]);
        l2.SetPtEtaPhiE(EleCand_et[Pair_lepton2[vtxind]],
                        EleCand_eta[Pair_lepton2[vtxind]],
                        EleCand_phi[Pair_lepton2[vtxind]],
                        EleCand_e[Pair_lepton2[vtxind]]);
          ...
          && EleTransientTracks.find(Pair_lepton2[vtxind])!=EleTransientTracks.end())
        {
          ...
          if (tr_vtx.isValid()) {
std::cout << "valid transient vertex found for electron-muon" << std::endl;
	...
	if (ele_match.size()<2) continue; // not enough electrons candidates on the vertex
        ...
        l1.SetPtEtaPhiE(EleCand_et[Pair_lepton1[vtxind]],
                        EleCand_eta[Pair_lepton1[vtxind]],
                        EleCand_phi[Pair_lepton1[vtxind]],
                        EleCand_e[Pair_lepton1[vtxind]]);
        l2.SetPtEtaPhiE(EleCand_et[Pair_lepton2[vtxind]],
                        EleCand_eta[Pair_lepton2[vtxind]],
                        EleCand_phi[Pair_lepton2[vtxind]],
                        EleCand_e[Pair_lepton2[vtxind]]);
         ...
         && EleTransientTracks.find(Pair_lepton2[vtxind])!=EleTransientTracks.end())
        {
          ...
          TransientVertex tr_vtx = fitter.vertex(trans_trks);
          if (tr_vtx.isValid()) {
std::cout << "valid transient vertex found for dielectrons" << std::endl;
            KalmanVertexCand_x[vtxind] = tr_vtx.position().x();
            KalmanVertexCand_y[vtxind] = tr_vtx.position().y();
            KalmanVertexCand_z[vtxind] = tr_vtx.position().z();
          }
...
std::cout << vtx.matchedMuons().size() << " muons on this vertex" << std::endl;
        for (unsigned int i=0; i<mu_match.size(); i++) {
          const int lep1 = mu_match.at(i).first;
        ...
        l1.SetPtEtaPhiE(MuonCand_pt[Pair_lepton1[vtxind]],
                        ...
                        MuonCand_phi[Pair_lepton1[vtxind]],
                        MuonCand_e[Pair_lepton1[vtxind]]);
        l2.SetPtEtaPhiE(MuonCand_pt[Pair_lepton2[vtxind]],
                        MuonCand_eta[Pair_lepton2[vtxind]],
                        MuonCand_phi[Pair_lepton2[vtxind]],
                        MuonCand_e[Pair_lepton2[vtxind]]);

        if (MuonTransientTracks.find(Pair_lepton1[vtxind])!=MuonTransientTracks.end()
         && MuonTransientTracks.find(Pair_lepton2[vtxind])!=MuonTransientTracks.end())
        {
std::cout << "found the associated transient tracks" << std::endl;
          std::vector<reco::TransientTrack> trans_trks;
          trans_trks.push_back(MuonTransientTracks[Pair_lepton1[vtxind]]);
          ...
          TransientVertex tr_vtx = fitter.vertex(trans_trks);
          if (tr_vtx.isValid()) {
std::cout << "valid transient vertex found for dimuons" << std::endl;
            KalmanVertexCand_x[vtxind] = tr_vtx.position().x();
            KalmanVertexCand_y[vtxind] = tr_vtx.position().y();
            KalmanVertexCand_z[vtxind] = tr_vtx.position().z();
          }
...
std::cout << "passed matching" << std::endl;
    ...
    LogDebug("GammaGammaLL") << "end of the loop on the vertices : vtxind = " << vtxind;
  }
  nExtraTracks = etind;

  LogDebug("GammaGammaLL") << "Passed Loop on vertices";
